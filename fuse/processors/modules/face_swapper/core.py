...
import fuse.choices
import fuse.jobs.job_manager
import fuse.jobs.job_store
from fuse import config, content_analyser, face_classifier, face_detector, face_landmarker, face_masker, face_recognizer, inference_manager, logger, state_manager, translator, video_manager
from fuse.common_helper import get_first, is_macos
from fuse.download import conditional_download_hashes, conditional_download_sources, resolve_download_url
from fuse.execution import has_execution_provider
from fuse.face_analyser import get_average_face, get_many_faces, get_one_face, scale_face
from fuse.face_helper import paste_back, warp_face_by_face_landmark_5
from fuse.face_masker import create_area_mask, create_box_mask, create_occlusion_mask, create_region_mask
from fuse.face_selector import select_faces, sort_faces_by_order
from fuse.filesystem import filter_image_paths, has_image, in_directory, is_image, is_video, resolve_relative_path, same_file_extension
from fuse.model_helper import get_static_model_initializer
from fuse.processors.modules.face_swapper import choices as face_swapper_choices
from fuse.processors.modules.face_swapper.types import FaceSwapperInputs
from fuse.processors.pixel_boost import explode_pixel_boost, implode_pixel_boost
from fuse.processors.types import ProcessorOutputs
from fuse.program_helper import find_argument_group
from fuse.thread_helper import conditional_thread_semaphore
from fuse.types import ApplyStateItem, Args, DownloadScope, Embedding, Face, InferencePool, ModelOptions, ModelSet, ProcessMode, VisionFrame
from fuse.vision import read_static_image, read_static_images, read_static_video_frame, unpack_resolution
...
def register_args(program : ArgumentParser) -> None:
	...
	if group_processors:
		...
		fuse.jobs.job_store.register_step_keys([ 'face_swapper_model', 'face_swapper_pixel_boost', 'face_swapper_weight' ])
...
def forward_swap_face(source_face : Face, target_face : Face, crop_vision_frame : VisionFrame) -> VisionFrame:
	...
	if is_macos() and has_execution_provider('coreml') and model_type in [ 'ghost', 'uniface' ]:
		face_swapper.set_providers([ fuse.choices.execution_provider_set.get('cpu') ])
