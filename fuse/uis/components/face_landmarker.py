...
import fuse.choices
from fuse import face_landmarker, state_manager, translator
from fuse.common_helper import calculate_float_step
from fuse.types import FaceLandmarkerModel, Score
...
def render() -> None:
	...
	FACE_LANDMARKER_MODEL_DROPDOWN = gradio.Dropdown(
		...
		choices = fuse.choices.face_landmarker_models,
	...
	FACE_LANDMARKER_SCORE_SLIDER = gradio.Slider(
		...
		step = calculate_float_step(fuse.choices.face_landmarker_score_range),
		minimum = fuse.choices.face_landmarker_score_range[0],
		maximum = fuse.choices.face_landmarker_score_range[-1]
	)
