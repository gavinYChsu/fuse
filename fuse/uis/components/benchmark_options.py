...
from fuse.types import BenchmarkMode, BenchmarkResolution
...
def render() -> None:
	...
	BENCHMARK_MODE_DROPDOWN = gradio.Dropdown(
		...
		choices = fuse.choices.benchmark_modes,
	...
	BENCHMARK_RESOLUTIONS_CHECKBOX_GROUP = gradio.CheckboxGroup(
		...
		choices = fuse.choices.benchmark_resolutions,
	...
	BENCHMARK_CYCLE_COUNT_SLIDER = gradio.Slider(
		...
		step = calculate_int_step(fuse.choices.benchmark_cycle_count_range),
		minimum = fuse.choices.benchmark_cycle_count_range[0],
		maximum = fuse.choices.benchmark_cycle_count_range[-1]
	)
